...
func init() {
	pterm.Println()
	str := pterm.DefaultHeader.WithBackgroundStyle(pterm.NewStyle(pterm.Bgblue)).WithMargin(10).Sprintf("project0200")

	pterm.DefaultCenter.Print(str)

	pterm.DefaultCenter.WithCenterEachLineSeparately().Printf(
		fmt.Sprintf("Created by %v %v\n%s[%s]%s", pterm.LightRed(devlopedByStyle("Hemanth Godavarthi")), pterm.yellow(devlopedByStyle("😀")), pterm.Yellow("contact"), pterm.Cyan("at"), pterm.Yellow("hemanth.godavarthi@gmail.com")))
